...
首页封面把高清源图（YouTube `maxresdefault` / 本地内嵌封面）以**等比例缩放**适配固定
30×17 的封面框：不超出、不裁切、不畸变。渲染方式按终端能力自动选择，也可用环境变量强制：

| 终端                          | 默认渲染           | 说明                                                                |
| ----------------------------- | ------------------ | ------------------------------------------------------------------- |
| kitty（及 ghostty / wezterm / rio） | 真图（协议内联） | 自动启用：封面按像素等比例显示，最清晰                              |
| foot / konsole / 其它         | 像素风（半块自绘） | 任何终端稳定；256 色 SGR，等比例居中，不依赖终端图形协议            |

**环境变量**（优先级最高，覆盖自动探测）：

| 变量                | 取值                        | 说明                                              |
| ------------------- | --------------------------- | ------------------------------------------------- |
| `MUSIC_TUI_COVER`   | `kitty` / `sixel` / `halfblocks` | 强制渲染方式；`sixel` 需在支持并持久显示的终端上使用（见下） |
| `MUSIC_TUI_CELL_W`  | 正整数（像素）              | 覆盖终端字符格宽度（协议真图尺寸换算用）          |
| `MUSIC_TUI_CELL_H`  | 正整数（像素）              | 覆盖终端字符格高度                                  |

示例：

```bash
MUSIC_TUI_COVER=sixel music-tui   # 在 konsole 等可持久显示图像的终端启用真图
MUSIC_TUI_COVER=halfblocks music-tui  # 强制像素风（任何终端）
```

> **为什么 sixel 默认不自动启用？** 多数终端（含 foot）的图像是“网格驻留”型——
> 图像区域被写入任何字符（如歌词行刷新）即被擦除，自动启用会导致封面随歌词闪烁/消失，
> 体验不可靠。kitty 的占位符协议为网格驻留设计（替换单元格、随文本滚动），因此自动启用。
> 若你的终端（如 konsole）能持久显示 sixel，用 `MUSIC_TUI_COVER=sixel` 显式开启。
...
| 键                                       | 功能                                       |
| ---------------------------------------- | ------------------------------------------ |
| `Tab` / `Shift+Tab`                     | 切换页面（循环）                           |
| `Ctrl+←` / `Ctrl+→`                     | 上一首 / 下一首（全局，任意页面生效）     |
| `Ctrl+Shift+←` / `Ctrl+Shift+→`         | 歌词时间 -0.5s / +0.5s（全局；同步改写本地 LRC 缓存，下次播放同一首生效） |
...
| `l`             | 添加本地音乐目录：自动新建「本地-<目录名>」列表并导入（仅目录） |
...
歌词识别错误： 修改~/.cache/music-tui/lyrics/ai.jsonl 中对应的条目
ai.jsonl 中没有问题，修改~/.cache/music-tui/lyrics/lrc 目录下吗对应的lrc文件， 文件名为ai.jsonl 中对应的 title-歌手名.lrc
