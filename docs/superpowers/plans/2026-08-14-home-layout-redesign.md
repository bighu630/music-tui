...
- [x] **Step 1: 写失败测试**——更新 `TestNextSequentialOrderAndStopAtEnd` 为回绕语义；新增：Prev 回绕、Prev 空队列、Prev 单曲队列、RepeatOne 模式 Next 正常推进、Shuffle 末尾回绕、Next 空队列不变。
- [x] **Step 2: 跑测试确认失败**：`go test ./queue/ -run 'TestNext|TestPrev'`
- [x] **Step 3: 实现**
...
- [x] **Step 4: 跑测试通过**：`go test ./queue/`
- [x] **Step 5: Commit**：`feat(queue): 三态播放模式与回绕 Next/Prev`
...
- [x] **Step 1: 写失败测试**：mpv 的 SetLoop 测试（在现有 mpv 测试基建上；若 mpv 测试用 fake conn，断言 `set_property loop-file` 调用）；fakePlayer 加 `SetLoop` 记录字段 `loops []bool`。
- [x] **Step 2: 确认失败**：`go build ./...`（接口未实现必然编译失败）
- [x] **Step 3: 实现**
...
- [x] **Step 4: 测试通过**：`go test ./player/ ./ui/`
- [x] **Step 5: Commit**：`feat(player): SetLoop 单曲无缝循环`
...
- [x] **Step 2: 确认失败**：`go test ./ui/ -run TestLineProgress -v`
- [x] **Step 3: 实现**（新文件 `ui/progressbar.go`）
...
- [x] **Step 4: 测试通过**：`go test ./ui/ -run 'TestLineProgress|TestProgressClick' -v`
- [x] **Step 5: Commit**：`feat(ui): 线条渐变进度条与点击换算`
...
- [x] **Step 3: 实现**
  ...
  - `,`/`.` 键 → 对应 cmd（无曲目忽略）。
- [x] **Step 4: 测试通过**：`go test ./ui/ -run 'TestHome' -v`
- [x] **Step 5: Commit**：`feat(ui): 首页全屏布局与底部控制区`
...
- [x] **Step 4: 测试通过**：`go test ./ui/`
- [x] **Step 5: Commit**：`feat(ui): 模式三态循环与上一首/下一首接线`
...
- [x] **Step 1: 定位受影响断言**：`grep -rn "播完\|StopAtEnd\|无下一首" queue/ ui/*_test.go`，逐一定位"播完停止"断言并更新为回绕语义（或按 spec 调整用例预期）。
- [x] **Step 2: 全量验证**
  - `go build ./...`
  - `go vet ./...`
  - `go test ./... -race`
- [x] **Step 3: 自查**：功能不回归——seek ←/→、空格暂停/重播、歌词高亮滚动、封面缓存渲染（TestHomeCover* 系列）、队列页 s 键。
- [x] **Step 4: Commit**：`test: 更新播完循环语义回归用例`
