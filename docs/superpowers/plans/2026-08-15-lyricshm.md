...
- [ ] **Step 4: root_test.go helper 传 nil**

`ui/root_test.go` 中 3 处 `newTestModel` / `newTestModelBase` / `newTestModelBaseWithCache` 对 `ui.NewModel(` 的调用末尾追加 `, nil`,以及 214/246/257 行 helper 之外若有其他直调 `NewModel` 的位置(搜索 `ui.NewModel(` 全部命中)同样追加 `, nil`。
