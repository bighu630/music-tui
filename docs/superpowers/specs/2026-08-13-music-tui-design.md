...
| 队列页 | `s` | 切换顺序/随机 |
| 队列页 | `m` | 进入移动模式（过滤确认态时先退出过滤；过滤输入框聚焦时是普通字符；队列 <2 首不进入） |
| 队列页（移动模式） | `↑↓←→`/`hjkl` | 移动当前光标下的歌曲：`↑/k` 上移一格、`↓/j` 下移一格、`←/h` 移到队首、`→/l` 移到队尾（每步实时生效，选中跟随被移动的歌曲） |
| 队列页（移动模式） | `Enter`/`Esc` | 结束移动模式（保留移动结果，两者等效；Enter 不触发跳转播放） |
| 首页 | — | 播放信息区显示 `位置/总数 · 模式`（如 `3/12 · 随机`） |

队列页展示：当前曲目 `▶` 标记 + 加粗高亮，全部条目带序号（`▶  1. 晴天`）；空队列显示空态提示。删除/清空不打断正在播放的 mpv（播完由 Next 自然衔接）。移动模式期间底部提示行切换为 `↑↓←→/hjkl 移动 · Enter/Esc 结束`，退出后恢复；普通提示行含 `m 移动`。移动后队列顺序更新，当前播放曲目索引跟随同一首歌（连播顺序 = 新数组顺序）；移动不打断正在播放的 mpv。

### 14.5 测试策略

| 模块 | 策略 |
|---|---|
| queue | 纯函数单测：Add/Replace/JumpTo/Remove/Move 四象限（Move：跨当前曲移动 currentIdx 跟随、移当前曲本身、边界非法）、Next 顺序推进与停止、SetMode 洗牌性质（集合不变/当前与前缀不动/显示序=播放序）、Clear 保留模式 |
| ui | tea.Program + update 驱动集成测试：Tab 五页循环（首页→队列→播放列表→搜索→历史）、Enter 替换语义、a 追加不打断、TrackEnded 自动连播与不切页、队列页跳转/删除/清空/模式切换/移动模式（进入退出、键位映射、边界、模态、过滤交互、currentIdx 跟随、移动后连播顺序）、首页位置显示 |
...
### 19.1 概述
现有歌词链路是确定性匹配（标题清洗 + lrclib /api/get ±2s / /api/search 30s 评分），对 YouTube 噪声标题（繁中、feat.、版本描述、频道名混入）命中率有限。本增强引入 **AI 判断主路径**（用户确认：所有歌词请求都走 AI 清洗 → 重查 lrclib，确定性匹配降级为兜底）：
...
### 19.3 匹配流程（AI 优先，用户确认：所有歌词请求都走 AI 判断）
```
播放 → AI 未配置？ → 纯确定性匹配（行为与无增强逐字节一致）
       ↓ 已配置
AI 识别（结果缓存优先 + single-flight；瞬时错误重试 1 次；成功即缓存含负缓存）
  ...
  └─ 识别成功 →
      ...
      未命中 → FetchForQuery 严格重查：/api/get 优先（lrclib ±2s）→
               失败 /api/search 严格评分（≤3s）→ 命中入歌词缓存并返回
      严格未命中 → 中文歌词源链（用户确认顺序：网易云 → QQ，匿名接口
