...
- 列表高度：过滤时不变（现布局 content = list(h-3) ≤ h-1 已余 2 行，加 1 行过滤行仍 ≤ h-1），无需改 setSize 的高度逻辑；但 setSize 需同步设置 filterInput.Width（`width-18`——前缀 "过滤: "6 列 + 计数 "(n/m)"≤10 列，下限 10）
- 提示行（bottomHint，同步更新；队列页前缀用 `modeLabel()` 反映真实模式——列表循环/随机播放/单曲循环）：
  - 聚焦中：队列 `{modeLabel} · 输入过滤 · Enter 确认 · Esc 取消`，历史 `输入过滤 · Enter 确认 · Esc 取消`
  - 已确认：队列 `{modeLabel} · Enter/p 跳转播放 · d 删除 · c 清空 · s 切换模式 · Esc 退出过滤`，历史 `Enter/p 重播 · d 删除 · c 清空 · a 添加到… · Esc 退出过滤`
