...
| toast 位置 | 右下角浮层：覆盖在状态栏上方一行的右端，不参与布局 |
...
- toast 覆盖 `lines[len-2]`（状态栏上方一行）右端：
  `keep = width - toastW - 2` 列截断原行（截断后追加 `\x1b[0m` 防样式渗透）
  + 两空格 + toast 文本；行数不变、其余内容不变 → 排版零跳动。
  超宽 toast 按 `m.width-2` 截断（预留分隔符空间，`ansi.TruncateLeft` 保留
  句尾动作语义），覆盖行恒不超窗口宽度，不会触发终端折行把 Tab 栏滚出屏幕。
...
- toast 活跃时对完整输出做右端覆盖（见布局节），不改变行数。
