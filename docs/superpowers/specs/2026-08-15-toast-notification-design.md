...
- 状态栏内容：左 = 当前曲目标题 `Title - Artist`（无播放时左段为 `未在播放`，
  无图标），右 = `⏵/⏸/⏹ 模式 · 队列位置 x/N`（播放中 `⏵`，暂停 `⏸`，
  播放结束/出错停止 `⏹`；仅播放时有右段）。渲染时右段优先完整，左段按
  剩余宽度 `width - rightW - 1` 动态 `ansi.Truncate` 截断（保左、尾部省略号）；
  极端窄窗口（右段宽 ≥ 窗口宽）右段截断兜底，恒 1 行不折行。首页控制栏已
  展示曲目信息，状态栏行留空（行恒存在，布局稳定）；其余页面（含选择器
  打开时）共用。
