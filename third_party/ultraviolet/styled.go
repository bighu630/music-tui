// Package uv — patched fork of github.com/charmbracelet/ultraviolet.
// PATCH: preserve zero-width APC (kitty ESC _ G ... ESC \) in StyledString.printString
// — upstream overwrote cell.Content with "=" for zero-width sequences, dropping
// the APC payload when it was concatenated with the placeholder U+10EEEE in the
// same cell. Fixed to "+=" so APC and placeholder coexist. See coverrender/kitty_uv_test.go.
// LICENSE: MIT (see third_party/ultraviolet/LICENSE).
package uv
...
func printString[T []byte | string](
	...
	for len(str) > 0 {
		...
		switch width {
		// PATCH: preserve zero-width APC (kitty ESC _ G ... ESC \) — upstream overwrote Content with =, fixed to +=
