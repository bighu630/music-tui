...
func (tq *TerminalQuerier) Query(query string, timeout time.Duration) (string, error) {
	...
	// 同步读取响应（select(2) 手动超时）：Go 的 SetReadDeadline 在
	// term.MakeRaw 之后的 fd 上失效（实测永久阻塞，见 csi.go 同款修复），
	// select 不受 termios 影响；同时避免原实现读 goroutine 无超时阻塞在
	// /dev/tty 上劫持 TUI 的按键输入。
	fd := int(tq.tty.Fd())
	...
	if err != nil {
		return "", wrapError("failed to read terminal query response", err)
	}
	if n <= 0 {
		return "", context.DeadlineExceeded
	}
