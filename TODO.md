...
- [x] 实现：lyrics/netease.go（搜索/歌词，毫秒→秒）+ qqmusic.go（搜索/歌词，需 Referer）+ fetchCN 链（时长 ≤3s 筛选、源错误继续、命中入 LRC 缓存）；EnhancedClient.EnableCNSources 显式启用（main 注入真源、测试注入 mock）
